import { createTodo, destroyTodo, updateTodo } from '../src/controller.js';

describe('add', () => {
  const todos = [];
  const todo = { description: 'create a watch', completed: false, index: 0 };
  test('todos length should be 1', () => {
    expect(createTodo(todo, todos)).toHaveLength(1);
  });

  test('index should be zero', () => {
    expect(createTodo(todo, todos)).toEqual(
...
describe('delete', () => {
  ...
  test('todos length should be 1', () => {
    expect(destroyTodo(todos[1], todos)).toHaveLength(1);
  });

  test('index should not be 1', () => {
    expect(destroyTodo(todos[1], todos)).not.toEqual(
